...
int _stepper(double (*f)(double, gsl_vector *, gsl_vector *),gsl_vector *in_state_vec, gsl_vector *out_state_vec,double x,double h,double xmax,double *h_next,double tolerance );
double _single_step(double (*f)(double, gsl_vector *, gsl_vector *), gsl_vector *in_vec, gsl_vector *out_vec,double x0,double h);
int *_vect_add(gsl_vector * sum, int vect_len, int count, ...);
int _populate_history(int hist_len, double history[hist_len][5], int step, double clock, gsl_vector *state);
int _arr2vec(int len, double in_arr[len], gsl_vector *out_vec);
void _integration_vector_init();
void _integration_vector_free();
void _print_vec(gsl_vector * vec);
...
/////////////////////
// _stepper steps the state from one timestamp to another.  It makes sure that the error
...
int _stepper(double (*f)(double, gsl_vector *, gsl_vector *),
   ...
   *h_next = h;
   ...
   _integration_vector_free();
   return 0;
}


////////////////
// _single_step computes the next value of the system for a single timestep.
...
// No return
///////////
void double_pendulum_eom(double t, gsl_vector *in_state, gsl_vector *out_state){
...
///////////////////
// _integration_vector_init initializes the integration_vectors struct
// _integration_vector_free frees the struct
// No arguments
////////////////////
void _integration_vector_init(){
    init_count+=1;
...
//////////////////////
// _print_vec prints a gsl_vector
...
/////////////////////
// _populate_history populates the history array
...
/////////////////
// _arr2vec converts a regular ol' array to a gsl_vector
//
// Arguments:
